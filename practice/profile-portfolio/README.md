...
> **과제 구분:** 추가과제
>
> **PDF 문서:** [추가과제 README PDF 보기](../../output/pdf/profile-portfolio-readme.pdf)
...
![About 탭](docs/images/about.png)

- `나에 대해`, `공부`, `기록`, `취미`, `소통` 항목을 탭으로 탐색합니다.
- 선택한 항목에 따라 설명, 세부 카드와 키워드가 동적으로 바뀝니다.
- 선택 상태와 변경되는 내용을 ARIA Tab과 Live 영역으로 보조 기술에 전달합니다.

### 프로젝트 경험

![프로젝트 경험](docs/images/projects.png)
...
![프로젝트 상세 모달](docs/images/project-modal.png)

- 현재 페이지를 벗어나지 않고 프로젝트 역할, 핵심 작업, 기술과 영상을 확인합니다.
- 모달이 열리면 배경 스크롤을 막고 Escape와 Tab 키로 닫기 및 포커스 순환을 제어합니다.
- 영상 재생이 불가능하면 안내 문구와 GitHub 대체 링크를 표시합니다.
...
| 다크모드 | 컬러 모드 |
| --- | --- |
| ![다크모드](docs/images/dark.png) | ![컬러 모드](docs/images/color-mode.png) |

- 다크모드는 어두운 배경과 밝은 텍스트로 전체 색상 변수를 전환합니다.
- 컬러 모드는 레이아웃을 유지하면서 버튼, 카드와 진행 상태의 포인트 색상을 강조합니다.
- 사용자가 선택한 모드는 Local Storage에 저장되어 다음 방문에도 유지됩니다.

### 모바일 내비게이션

![모바일 내비게이션](docs/images/mobile-menu.png)

- 작은 화면에서는 가로 메뉴를 햄버거 버튼과 세로 메뉴로 전환합니다.
- 주요 섹션 이동과 다크·컬러 모드를 모바일 메뉴 안에서도 사용할 수 있습니다.
- 메뉴의 열림·닫힘 상태를 `aria-expanded`로 전달하고 메뉴 선택 후 자동으로 닫습니다.
...
프론트엔드 화면과 상호작용을 중심으로 구현하되, 방문자가 실제로 기록을 공유하고 온라인에서 사이트를 확인할 수 있도록 Supabase와 AWS CloudFront를 보조적으로 사용했습니다.

| 기술 | 사용 목적 | 선택 이유 | 고려할 점 |
| --- | --- | --- | --- |
| Supabase | Daily Log, 방명록과 댓글 저장 | Local Storage와 달리 여러 방문자가 같은 데이터를 조회하고 작성할 수 있으며 정적 프론트엔드에서도 비교적 간단하게 연결 가능 | 인터넷 연결이 필요하고 요청 중·실패 상태를 화면에서 처리해야 함 |
| AWS CloudFront | 완성된 정적 사이트 공개 | 별도의 애플리케이션 서버 없이 HTML, CSS, JavaScript와 미디어 파일을 온라인 주소로 제공 가능 | 파일을 수정한 뒤 이전 캐시가 남을 수 있어 새 버전 반영 여부를 확인해야 함 |

Supabase는 데이터베이스 기술 자체를 보여주기보다 Daily Log와 방명록이 실제 방문자 사이에서 공유되도록 사용했습니다. 프론트엔드에서는 캘린더와 목록 렌더링, 입력 검증, 저장 이후 화면 갱신, Toast와 오류 안내에 집중했습니다.

CloudFront는 프로젝트를 로컬 파일로만 제출하지 않고 누구나 같은 주소에서 실행 결과를 확인할 수 있게 하기 위해 선택했습니다. 배포 인프라를 핵심 기능으로 다루기보다는 완성된 프론트엔드 결과물을 공유하는 실행 환경으로 활용했습니다.
...
자유주제로 구성하는 방식이라 시간이 부족해 아직 저에 대해 제대로 이야기 한적이 없는 것 같아 저에 대한 정보와 개발자 이력서형 포트폴리오 방향을 섞는 형태로 구성했습니다. 프로젝트와 기술 스택만 보여주기보다 성격, 학습 과정, 취미와 하루 기록을 함께 담아 김지원이라는 사람을 알아갈 수 있는 개인 블로그를 만들고자 했습니다. 개발을 준비하는 사람들과 소통하는 공간이라는 목적은 유지하면서도 개발 이야기만으로 화면을 채우지 않고 일상적인 내용과 참여 기능을 균형 있게 구성했습니다.
